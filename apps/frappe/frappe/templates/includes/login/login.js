...
login.reset_sections = function (hide) {
	// Sửa để không ảnh hưởng đến UI custom - bỏ toggle sections
...
login.login = function () {
	...
	// Bỏ toggle để không ảnh hưởng UI custom
...
var request_otp = function (r) {
	// Sửa để không làm hỏng UI - chỉ xóa form bên trong
	$('.login-content form').empty();
	$('.login-content form').append(
		`<div id="twofactor_div">
			...
			<button class="btn btn-primary mt-3" id="verify_token">{{ _("Verify") | e }}</button>
		</div>`
	);
