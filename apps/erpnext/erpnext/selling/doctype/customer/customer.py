...
class Customer(TransactionBase):
	...
	def on_update(self):
		...
		self.create_address_from_custom_fields()  # Add auto address creation
	...
	def create_address_from_custom_fields(self):
		"""Create address from custom_address and custom_city fields"""
		from frappe.contacts.doctype.address.address import get_address_display
		
		# Check if custom fields are filled and no primary address exists
		if (self.get("custom_address") and self.get("custom_city") and 
			not self.get("customer_primary_address")):
			
			try:
				# Prepare address data from custom fields
				address_info = frappe._dict({
					"doctype": "Customer",
					"name": self.name,
					"customer_name": self.customer_name,
					"address_line1": self.custom_address,
					"address_line2": "",  # Empty since we only have one address field
					"city": self.custom_city,
					"state": self.custom_city,  # Use city as state if no separate field
					"country": "Vietnam",  # Default country, can be customized
					"pincode": "",
				...
				frappe.msgprint(
					_("Address automatically created from custom fields: {0}").format(address.name),
					title=_("Auto Address Created"),
					indicator="green"
				)
				
			except Exception as e:
				frappe.log_error(f"Error creating address from custom fields: {str(e)}")
				# Don't throw error, just log it to avoid blocking customer save
...
@frappe.whitelist()
def make_customer_address(customer_name, address_data):
	...
	try:
		...
		# Get customer doc to verify it exists
		customer = frappe.get_doc("Customer", customer_name)
		
		# Prepare address data with customer info
		address_info = frappe._dict({
			"doctype": "Customer",
			"name": customer.name,
		...
		# Create address using the existing function
		address = make_address(
			address_info,
			is_primary_address=address_data.get("is_primary_address", 0),
			is_shipping_address=address_data.get("is_shipping_address", 1)
		)
		...
		return {
			...
			"message": f"Address created successfully for customer {customer.customer_name}"
		}
		
	except Exception as e:
		frappe.log_error(f"Error creating customer address: {str(e)}")
