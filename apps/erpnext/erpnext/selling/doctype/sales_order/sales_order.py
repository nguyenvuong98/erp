...
def make_delivery_note(source_name, target_doc=None, kwargs=None):
	...
	mapper = {
		"Sales Order": {
			...
			"field_no_map": [
				"customer_name",  # Không copy customer_name, để fetch từ customer mới
				"tax_id",        # Không copy tax_id, để fetch từ customer mới
			]
		},
	...
	def set_missing_values(source, target):
		...
		target.update({"custom_e_discount_amount": source.discount_amount})
		target.update({"apply_discount_on": "Net Total"})
	...
	def update_item(source, target, source_parent):
		...
		#add valueation_rate = Don gia von
		target.custom_valuation_rate = flt(source.valuation_rate)
		
		# Calculate discount amount: custom_discount = (custom_p_discount / 100) * amount
		if target.amount and source.custom_p_discount:
			target.custom_discount = (flt(source.custom_p_discount) / 100) * flt(target.amount) 
		else:
			target.custom_discount = 0
		
		# Calculate e_amount: custom_e_amount = amount - custom_discount
		target.custom_e_amount = flt(target.amount) - flt(target.custom_discount)
		
		# Calculate profit percentage: custom_p_profit = ((custom_e_amount - custom_valuation_rate) / custom_valuation_rate) * 100
		if target.custom_e_amount and target.custom_valuation_rate and target.custom_valuation_rate != 0:
			profit_amount = flt(target.custom_e_amount) - (flt(target.custom_valuation_rate) *flt(target.qty))
			dongiavon_sl= (flt(target.custom_valuation_rate) *flt(target.qty))
			target.custom_p_profit = (profit_amount / dongiavon_sl) * 100
		else:
			target.custom_p_profit = 0
		...
		source_name,
		{
			"Sales Order": {
				...
				"field_map": {
					...
					"custom_end_customer": "customer",
				},
				"field_no_map": ["payment_terms_template", "customer_name", "tax_id"],
				"validation": {"docstatus": ["=", 1]},
			},
			"Sales Order Item": {
