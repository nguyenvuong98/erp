...
class Item(Document):
	...
	def after_insert(self):
		...
		# 💰 Custom logic: Handle custom_whole_price for new items
		self.handle_custom_whole_price()
	...
	def on_update(self):
		...
		# 💰 Custom logic: Auto create/update Item Price from custom_whole_price
		self.handle_custom_whole_price()
	...
	def handle_custom_whole_price(self):
		"""
		💰 Custom method: Auto create/update Item Price when custom_whole_price is set
		
		Logic:
		1. Check if custom_whole_price field exists and has value
		2. Get default selling price list from Selling Settings
		3. Check if Item Price already exists for this item and price list
		4. If exists: Update the price_list_rate
		5. If not exists: Create new Item Price record
		
		This method is called in on_update() to automatically sync
		custom_whole_price with Item Price records.
		"""
		# 🔍 Check if custom_whole_price field exists and has value
		if not hasattr(self, 'custom_whole_price') or not self.custom_whole_price:
			return
			
		# 💡 Skip if price is zero or negative
		if flt(self.custom_whole_price) <= 0:
			return
			
		# 📋 Get default selling price list from settings
		default_price_list = frappe.db.get_single_value("Selling Settings", "selling_price_list")
		if not default_price_list:
			# 🔄 Fallback to "Standard Selling" if no default set
			default_price_list = frappe.db.get_value("Price List", {"selling": 1}, "name")
			
		if not default_price_list:
			# ⚠️ No selling price list found, skip
			frappe.log_error(f"No selling price list found for item {self.name}")
			return
			
		# 🔍 Check if Item Price already exists
		existing_item_price = frappe.db.get_value("Item Price", {
			"item_code": self.name,
		...
		if existing_item_price:
			# 🔄 Update existing Item Price if different
			existing_price_name, existing_rate = existing_item_price
			
			if flt(existing_rate) != flt(self.custom_whole_price):
				frappe.db.set_value("Item Price", existing_price_name, "price_list_rate", self.custom_whole_price)
				
				# 📝 Log the update
				frappe.msgprint(
					_("Item Price updated: {0} → {1} in {2}").format(
						frappe.format_value(existing_rate, "Currency"),
						frappe.format_value(self.custom_whole_price, "Currency"),
						default_price_list
					),
			...
			try:
				new_item_price = frappe.get_doc({
					...
					"item_name": self.item_name,
					"price_list": default_price_list,
					"price_list_rate": self.custom_whole_price,
					"selling": 1,
					"buying": 0,
					"currency": erpnext.get_default_currency(),
					"uom": self.stock_uom
				...
				# 📝 Log the creation
				frappe.msgprint(
					_("Item Price created: {0} = {1}").format(
						default_price_list,
						frappe.format_value(self.custom_whole_price, "Currency")
					),
					title=_("Price Created"),
					indicator="green"
				)
				
			except Exception as e:
				# 🚫 Log error if creation fails
				frappe.log_error(f"Error creating Item Price for {self.name}: {str(e)}")
				frappe.msgprint(
					_("Error creating Item Price: {0}").format(str(e)),
					title=_("Error"),
					indicator="red"
				)
