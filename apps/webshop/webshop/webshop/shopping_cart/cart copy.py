...
@frappe.whitelist()
def update_cart(item_code, qty, additional_notes=None, with_items=False, customer=None, shipping_address_name=None, add_qty=False):
	quotation = _get_cart_quotation()

	# Handle selected customer update
	if customer:
		quotation.custom_end_customer = customer
		quotation.flags.ignore_permissions = True
		quotation.save()
		return {"name": quotation.name, "message": "Selected customer updated successfully"}
	...
	if shipping_address_name:
		...
		quotation.flags.ignore_permissions = True
		quotation.save()
		return {"name": quotation.name, "message": "Shipping address updated successfully"}
